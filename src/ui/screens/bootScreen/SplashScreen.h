    ...
    public:
        ...
        void update() override {
            ...
            if (!_started) {
                _started   = true;
                _startTime = millis();
                Serial.println(">>> Splash first frame at ms: " + String(millis()));
            }
            unsigned long elapsed = millis() - _startTime;
            Serial.println("elapsed: " + String(elapsed) + " phase: " + String(_phase));
            ...
            if (_phase == 0) {
                _letterCount = min((int)((elapsed) / 130), titleLen);
            } else {
