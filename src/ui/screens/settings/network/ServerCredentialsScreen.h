...
public:
    ...
    void onEnter() override {
        ...
        if (_host.isEmpty()) _host    = _ws.host();
        if (_port.isEmpty()) _port    = String(_ws.port());
        if (_path.isEmpty()) _path    = _ws.path();
        _dirty = true;
    }
    ...
    void onLongPressSelect() override {
        ...
        _ws.saveConfig(_host, port, _path.isEmpty() ? "/ws" : _path);
        _wantsPop = true;
    }
...
private:
    ...
    void _pushHostPicker() {
        ...
        _ui.pushScreen(new CharPickerWidget(
            ...
            [this](const String& r) { _host = r; _dirty = true; }
        ));
    }
    ...
    void _pushPortPicker() {
        ...
        _ui.pushScreen(new CharPickerWidget(
            ...
            [this](const String& r) { _port = r; _dirty = true; }
        ));
    }
    ...
    void _pushPathPicker() {
        ...
        _ui.pushScreen(new CharPickerWidget(
            ...
            [this](const String& r) { _path = r; _dirty = true; }
        ));
    }
    ...
    void _draw() {
        ...
        String      values[FIELD_COUNT] = {
            _host.isEmpty() ? "(not set)" : _host,
            _port.isEmpty() ? "3000"      : _port,
            _path.isEmpty() ? "/ws"       : _path
        };
