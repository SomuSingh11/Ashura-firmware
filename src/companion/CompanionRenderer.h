...
#include <Arduino.h>
#include <U8g2lib.h>
#include "../src/companion/Eyeparams.h"
#include "../src/companion/MoodEngine.h"
...
public:
    ...
    void draw(U8G2& u, int rx, int ry, int rw, int rh) {
        ...
        int eW = max(8,  (int)(rw * 0.30f));
        int eH = max(6,  (int)(rh * 0.56f));

        // Apply squish from params
        int sW = max(6, (int)(eW * _cur.squishX));
        int sH = max(4, (int)(eH * _cur.squishY));

        // Corner radius — scales with eye size, capped so it stays rounded
        int cr = max(2, min(sW, sH) / 3);
...
private:
    ...
    void _scheduleNextBlink() {
        _nextBlink = millis() + 2500 + (unsigned long)random(0, 3000);
    }

    void _tickBlink(unsigned long now) {
        switch (_blinkSt) {
            case BlinkSt::OPEN:
                if (now >= _nextBlink) {
    ...
    unsigned long _microStart = 0;
    unsigned long _nextMicro  = 0;

    void _scheduleNextMicro() {
        _nextMicro = millis() + 2000 + (unsigned long)random(0, 5000);
    }

    void _tickMicro(unsigned long now) {
                  ...
                  float pSize, float pX, float pY)
    {
        ...
        int   pr           = max(2, (int)(pSize * eH * 0.36f));
        ...
        px = constrain(px, cx - maxPX, cx + maxPX);
        py = constrain(py, cy - maxPY, cy + maxPY);
